...
# Файл, который есть в репозитории и которого нет в образе. Годится как
# признак: без него нельзя проверить ни одну договорённость про запуск.
REPOSITORY_MARKER = PROJECT / "docker-compose.app.yml"


def repository_is_present() -> bool:
    """
    Отличает работу из репозитория от работы внутри образа.

    Образ копирует только то, что нужно работе: пакеты, схему индекса,
    языковые модели и сам набор тестов. Файлов сборки, compose и юнитов
    systemd в нём нет — и не должно быть, класть их туда незачем.

    Из-за этого роль `tests` внутри образа никогда не была зелёной: два
    десятка проверок читают `docker-compose.app.yml`, `systemd/` и
    `Dockerfile`, а в контейнере этих файлов не существует. Выглядело
    это как двадцать сломанных тестов, хотя сломано не было ничего:
    проверки просто спрашивали про файлы, которых там нет по замыслу.

    Такие проверки объявляют пропуск. Пропуск виден в выводе и говорит
    правду: договорённости про запуск проверяются там, где есть чем.
    """

    return REPOSITORY_MARKER.is_file()


def requires_repository(subject):
    """Пропускает проверку, если рядом нет файлов репозитория."""

    return unittest.skipUnless(
        repository_is_present(),
        "нужен репозиторий: в образе нет файлов сборки, compose и systemd",
    )(subject)
