...
class SchemaContractTests(unittest.TestCase):
    ...
    def test_graph_filter_is_flattened_on_index_side(self) -> None:
        ...
        Обратный индекс Lucene хранит позиционные приращения, но не длину
        токена в позициях. Слитный «512р» занимает две позиции, и без
        выпрямления поток записывается со сдвигом — соседние токены
        встают не туда, и часть написаний перестаёт находиться.

        Проверяется состав анализатора: сам результат выпрямления по
        `positionLength` не виден, `flatten_graph` его не меняет. Это
        стоило одной неверной проверки в `ops/validate_elasticsearch.py`,
        которая падала на исправной схеме.
        """
