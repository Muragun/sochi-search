...
### Перечитать документы, когда изменился разбор

Если поменялся не сайт, а способ разбора — добавили извлечение реквизитов,
научились отличать машинное имя файла, — документы в индексе остаются со
старыми полями. Сдвинуть срок проверки мало: воркер сравнит хеш, увидит
совпадение и выйдет, не переразбирая.

```bash
# посмотреть, сколько подпадает
python -m ops.requeue --database data/crawl_state.sqlite3 \
    --office --missing-requisites
...
Сбрасываются хеш содержимого и условные заголовки: иначе сервер ответит
304 и содержимое не доедет. HTML-страницы по умолчанию не затрагиваются —
их больше всего, и перечитывать их заодно значит поставить в очередь
...
330 тестов, пять пропускаются: они описывают снятый движок распознавания
...
| `requeue.py` | 290 | перечитать документы, когда изменился разбор |
...
tests/                             330 тестов на стандартном unittest
