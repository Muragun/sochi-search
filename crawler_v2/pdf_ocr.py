...
# v7 — распознавание только по-русски, лестница повторов по собственной
# уверенности Tesseract (мягкая бинаризация, затем повороты) и починка
# текста: склейка переносов, латинские двойники кириллицы.
#
# v6 — внешний Tesseract с бинаризацией Сауволы, устранением перекоса и
# рендером в родном разрешении скана.
#
# Строку обязательно поднимать при любом изменении, влияющем на результат
# распознавания: именно по ней фоновый проход отбирает документы, которые
# нужно переделать. Тест сверяется с этой константой, а не с её значением.
PDF_EXTRACTION_VERSION = "pdf-ocr-v7"
...
class PdfPageText:
    ...
    #
    # Ноль означает «не измерялась» — так у страниц с готовым текстовым
    # слоем, которые вообще не проходили через OCR.
