...
Около 80 тысяч адресов в очереди, 171 тысяча фрагментов текста в
индексе, 13 тысяч PDF — из них большая часть сканы без текстового
слоя.
...
Ниже — что получилось и почему приняты такие решения. Подробный
разбор устройства, по слоям и по файлам, — в
...
| **Проверки** | GitHub Actions: 3.12, 3.8 и сборка образа |

Размер: около 24 тысяч строк Python, 8 тысяч строк тестов и 5 тысяч
строк интерфейса. Тестов 375, прогон на каждое изменение.
...
375 тестов, из них 5 пропускаются: все пять описывают снятый движок
...
Библиотеки для этого ставить не нужно: набор запускается и на голой
копии репозитория, а тесты, которым нужен numpy или PyMuPDF,
объявляют пропуск сами. Это проверяется отдельным тестом — обещание
уже нарушалось, и два модуля вместо пропуска давали ошибку загрузки.

То же самое запускается само, на каждое изменение:
[.github/workflows/ci.yml](.github/workflows/ci.yml). Четыре прогона —
3.12 с зависимостями (контейнер), 3.12 без них (свежая копия), 3.8
(версия рабочего сервера) и сборка образа.
...
ops/             перенос индекса, резервные копии, снимки, проверки кластера
docker/          точка входа образа: одна роль на контейнер
systemd/         юниты для установки без Docker
tests/           375 тестов на стандартном unittest
.github/         прогон проверок на каждое изменение
...
| `ops/snapshot.py` | ежедневные снимки индекса и уборка старых |
...
| Файл | О чём |
| --- | --- |
| [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) | как всё устроено: слои, форматы, схема индекса, поиск, файлы |
...
| [docs/AUDIT_ROUND_3.md](docs/AUDIT_ROUND_3.md) | третий круг: чего не хватало для эксплуатации |
...
Лицензия — MIT, текст в [LICENSE](LICENSE). Языковые модели Tesseract в
`ocr/tessdata/` — чужая работа со своим копирайтом, тоже под MIT:
происхождение и текст в
[ocr/THIRD_PARTY_NOTICES.md](ocr/THIRD_PARTY_NOTICES.md).
