...
- **Пакет, который выглядит мёртвым, но работает.** Первая версия
  сервиса, `app/`, десять модулей: её не собирает Dockerfile, на неё не
  ссылаются ни тесты, ни README, и в `systemd/` её юнита нет. По этим
  признакам она мёртвая — и была удалена как мёртвая. На сервере она
  живая: юнит `sochi-search.service` заведён на машине руками и поднимает
  её на отдельном порту рядом с `app_v2`. Пакет возвращён.

  Отсюда две проверки. Первая ловит появление пакета Python вне списка
  проверяемых — раньше проверка связок перебирала три имени, и четвёртый
  пакет проходил мимо неё целиком. Вторая сверяет поля, по которым ищет
  старый API, со схемой индекса: он спрашивает тот же alias, и
  переключение на новую схему молча оставило бы его с пустой выдачей.

  Общий вывод неприятный, но полезный: «в репозитории ни одной ссылки»
  не значит «не используется». Юнит, заведённый на сервере руками, из
  репозитория не виден.
...
Размер: около 23 тысяч строк Python, 8 тысяч строк тестов и 5 тысяч
строк интерфейса. Тестов 277.
...
277 тестов, из них 5 пропускаются: все пять описывают снятый движок
...
tests/           277 тестов на стандартном unittest
