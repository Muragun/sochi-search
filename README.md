...
- **Пакет, о котором забыли.** Первая версия сервиса, `app/`, десять
  модулей, оставалась в репозитории после переезда на `app_v2`: её не
  собирал Dockerfile, не запускал ни один юнит, а `.env.example`
  документировал её переменные, которых в текущем коде нет. Проверка
  неподключённых модулей её не видела — она перебирает три пакета по
  именам, и четвёртый проходит мимо неё целиком. Добавлен тест, который
  ловит появление пакета вне списка проверяемых.
