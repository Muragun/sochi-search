...
**Каталог снимков должен принадлежать кластеру.** `path.repo` разрешает
Elasticsearch писать в каталог, но не делает каталог доступным для
записи: свежий том Docker создаётся от root, а Elasticsearch в образе
работает под uid 1000. Без этого первый же запуск отвечает `path is not
accessible on master node` — фраза, которая читается как «путь не тот»,
хотя путь ровно тот.

```bash
docker volume ls | grep snapshots
sudo docker run --rm -v sochi-search_essnapshots:/snapshots alpine \
    chown 1000:0 /snapshots
```

Перезапускать кластер после этого не нужно.

**Место на диске.** Хранилище лежит в томе `essnapshots`, на том же
диске, что и индекс, и первый снимок стоит как весь индекс. На рабочей
машине это 488 МБ при 171 627 документах — то есть немного, но
посмотреть стоит: цифра растёт вместе с корпусом.
