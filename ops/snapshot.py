...
#
# Три, а не семь: осторожное значение, которое можно поднять, посмотрев
# на диск. Хранилище снимков лежит там же, где индекс, первый снимок
# стоит как весь индекс, а следующие дописывают только новые сегменты —
# но пока идёт фоновое распознавание, документы переписываются, сегменты
# сливаются, и «только новые» получается заметно больше, чем на
# неподвижном индексе.
#
# На рабочей машине запас большой: индекс 488 МБ при 171 627 документах
# против 58 ГБ свободного места, и семь снимков там ничем не грозят.
# Умолчание рассчитано не на неё, а на установку, про которую ничего не
# известно: заполнить диск копиями — ровно та беда, от которой в этом же
# проходе закрыли журналы контейнеров.
DEFAULT_RETENTION = 3
DEFAULT_PREFIX = "auto"
...
def ensure_repository(
    ...
    *,
    ...
    try:
        ...
        # root, а Elasticsearch в официальном образе работает под uid
        # 1000 — и проверка хранилища падает на попытке создать в нём
        # пробный объект.
        #
        # Само сообщение кластера про это молчит: «path is not
        # accessible on master node» звучит как «путь не тот», хотя путь
        # ровно тот, и `access_denied_exception` в глубине ответа
        # заметить нетрудно, а связать с владельцем тома — уже нет.
        if (
            "access_denied" in str(error)
            or "is not accessible" in str(error)
        ):
            raise SnapshotError(
                "{0}\n\n"
                "Кластеру разрешено писать в {1}, но сам каталог ему "
                "не принадлежит. Свежий том Docker создаётся от root, а "
                "Elasticsearch в образе работает под uid 1000.\n"
                "  sudo docker run --rm -v <том>:/snapshots alpine \\\n"
                "      chown 1000:0 /snapshots\n"
                "Имя тома: docker volume ls | grep snapshots. "
                "Перезапуск кластера после этого не нужен.".format(
                    error, location
                )
            ) from error
